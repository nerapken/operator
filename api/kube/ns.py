from requests import get, post

class Namespace:
    
    def __init__(self) -> None:
       self.header = {'Authorization': 'Bearer eyJhbGciOiJSUzI1NiIsImtpZCI6IjEwWHFpZGhxdmxpTEFONGVfUV9mYXd0WDVfNk1Ta1lnSGxES1pfNmlla1kifQ..dfH6avQuan2tEAvdDhOOEEy6DotMHhquDHCENgIOQNMqy_O9USqNAU7Wec0SOWl4xQ4xzkba25aME4tPkHPo4fvKQk38ddeSTrchlK0welqovDHkqtuhagOkQSH4dyCaHtZ7tITjMgRaEaa43ZtjMOwFaPcIbKjL3uIxoOSgAmXW-zL1_tO7L7MbKoC1XiG3jsyvI1lCQpewnVWHL6rzay-rMO65dV4pcXLECRdWb-RoZFodxG0ZriXmLUMukfykAEV6vmxqioMxSjYx7CbngYABi7eO6tY1C2mSygMljEtnAGQmHhsfmTZNeTGEFjuacoYZ9bJA_0jyTimqvAAR_g'}
       self.cert = '/home/aji/ca-nerapken.pem'
       self.url = 'https://172.31.4.147:6443'
       self.userid = 'admin'
       self.ns_path = '/api/v1/namespaces/'
       self.data = {'kind': 'Namespace', 'apiVersion': 'v1', 'metadata': {'name': 'nerapken-test'}}


    def ns_get(self):
        r = get(url=self.url+self.ns_path, headers=self.header, verify=self.cert)
        return r.json()

    def ns_create(self):
        r = post(url=self.url+self.ns_path, headers=self.header, json=self.data, verify=self.cert)
        return r.json()

    
n = Namespace()
print(n.ns_create())